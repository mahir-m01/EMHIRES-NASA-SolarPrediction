...
The complete shared state type is defined as:
...
class GridAdvisorState(TypedDict):
    ...
    model_name: str
    forecast_date: str
    weather_forecast: list      # raw Open-Meteo 24h response
    cf_value: float             # peak CF from hourly profile
    hourly_profile: list        # 24 CF values (one per hour)
    monthly_profile: list       # 12 monthly CFs at peak hour conditions
    risk_summary: dict          # variability_score, low_hours, ramp_events
    risk_flags: list            # plain-English risk descriptions
    retrieved_chunks: list      # top-4 RAG chunks with source labels
    final_recommendations: dict # LLM JSON output
    error: Optional[str]
\end{lstlisting}
...
# Slice to the requested forecast day (day_offset * 24 hours into the response)
start = day_offset * 24
irr = hourly["shortwave_radiation"][start:start+24]
tmp = hourly["temperature_2m"][start:start+24]
wnd = hourly["wind_speed_10m"][start:start+24]
\end{lstlisting}
...
def risk_analysis_node(state: GridAdvisorState) -> dict:
    ...
    if not profile:
        return {
            "risk_summary": {"variability_score": 0.0,
                             "low_hours": [], "ramp_events": []},
    ...
    ramp_events = [h for h in range(1, 24)
                   if abs(profile[h] - profile[h - 1]) > 0.12]

    risk_flags = []
    if ramp_events:
        risk_flags.append(
            f"High variability window: hours {ramp_events[0]}-{ramp_events[-1]}")
    if low_hours:
        risk_flags.append(
            f"Minimal generation risk: {len(low_hours)} hours below 5% capacity")
...
At inference time, the query is constructed from the pipeline state as:
...
The system prompt enforces three constraints simultaneously: output format (JSON schema), audience (grid operator), and domain boundaries (no consumer-facing language, no nighttime risk flagging). Key excerpts:

\begin{lstlisting}[language=Python]
_SYSTEM_PROMPT = """You are a grid integration advisor for European solar energy systems.
Your audience is grid operators, TSO/DSO planners, and energy system managers
-- not consumers.
...
Output must match this exact structure exactly:
{
  ...
  "risk_periods": [
    {"period": "string", "start_hour": 0, "end_hour": 3,
     "risk": "string", "severity": "high|medium|low"}
  ],
  "strategies": [
    {"title": "string", "description": "string",
     "category": "grid_balancing|storage|demand_response|market",
     "source": "retrieved|general"}
  ],
...
- Compute peak power as: Installed Capacity (kW) x Peak CF.
  Use this exact kW value -- do not estimate or round differently.
- State generation window hours, peak kW output, and variability level.

RISK PERIOD rules:
- Always flag the evening ramp-down when peak CF > 0.05. Find the exact
  hour output starts declining after the daily peak; set start_hour there,
  end_hour where CF nears zero.
- NEVER flag nighttime (20:00-06:00) or morning ramp-up (06:00-13:00).
- start_hour must be less than end_hour. Max 5-hour window.
- severity: high = variability_score > 0.25 or mid-day CF drop >30%;
  medium = rapid evening ramp-down; low = minor fluctuation.
- For variability_score < 0.10: only the evening ramp-down at medium severity.

STRATEGY rules -- grid operator / TSO level ONLY:
- Priority: (1) Reserve activation & grid balancing, (2) BESS dispatch,
  (3) Curtailment vs storage trade-offs, (4) Intraday market participation.
- Reference: PICASSO aFRR, MARI mFRR, SIDC intraday trading,
  ENTSO-E balancing platforms, SNSP limits, FCR/FFR procurement.
- category: "grid_balancing" for reserve/frequency, "storage" for BESS,
  "demand_response" for load management, "market" for trading.
- Do NOT recommend consumer-level actions (EV scheduling, pre-cooling).

REFERENCES rules:
- List 2-4 specific sources cited in your strategies.
- Only cite sources present in the retrieved guidelines.
