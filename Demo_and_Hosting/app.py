...
model = get_model(AVAILABLE_MODELS[model_choice])
...
with tab_forecast:
    ...
    with r2c2:
        ...
        categories = ["Irradiance", "Solar Angle", "Season", "Temperature", "Wind"]
...
**Feature extraction** — Raw timestamps are split into numeric `Hour` (0-23) and `Month` (1-12) columns that the model can use directly.
    ...
    },
    {
        ...
        "tag": "Encoding",
        "description": """
Each observation is transformed into a **33-dimensional feature vector**:

| Type | Features | Count |
|------|----------|-------|
| Continuous | GHI, Temperature, Wind Speed | 3 |
| Temporal | Hour, Month | 2 |
| Categorical | Country (one-hot encoded) | 28 |
| **Total** | | **33** |

One-hot encoding converts each country code (e.g. `ES`) into 28 binary columns (`Country_ES = 1`, rest `= 0`). This lets the model learn country-specific offsets based on latitude, climate, and cloud patterns.
...
with tab_pipeline:
    ...
    with chart_col:
        ...
        if current == 0:
            ...
            features = ["Country (×28)", "Wind Speed", "Month", "Temperature", "Hour of Day", "Irradiance (GHI)"]
...
4. **Encoding** — One-Hot encoding (28 countries)
